...
- Как минимум у одного вопроса варианты ответов должны содержать дистракторы — типичные ошибки учеников, правдоподобные, но неправильные варианты.
"""
...
5a. **no_distractors** (warning) — среди вариантов ответа нет дистракторов: все неправильные варианты очевидно ложные или абсурдные. Хотя бы в одном вопросе викторины должны быть правдоподобные дистракторы — варианты, которые отражают типичные ошибки учеников данного класса по этой теме.
...
- **warning**: вопрос можно использовать, но желательно поправить (ambiguous, duplicate_options, no_distractors, difficulty_mismatch).
...
- Если среди проблем есть no_distractors — добавь правдоподобные дистракторы (неправильные варианты, отражающие типичные ошибки учеников), но не делай их абсурдными или очевидно ложными.
"""